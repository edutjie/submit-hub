    ...
    <div className="w-full bg-white border-[2px] border-light-gray rounded-[20px] p-[32px] flex flex-col transition-shadow hover:shadow-[0_4px_12px_rgba(0,0,0,0.05)]">
      <div className="flex flex-col lg:flex-row justify-between w-full gap-[32px]">
        ...
        <div className="flex flex-col text-left w-full lg:max-w-[60%]">
          <h3 className="text-h3 font-semibold text-black mb-[10px]">{assignment.name}</h3>
        ...
        {/* Interaction section */}
        <div className="flex flex-col sm:flex-row flex-wrap lg:flex-nowrap items-start sm:items-center lg:items-start gap-[22px] shrink-0 w-full lg:w-auto">
          ...
          <div className="w-full sm:w-auto flex flex-col sm:flex-row">
            {isSubmitted ? (
              <Button variant={2} className="w-full sm:w-auto justify-center" onClick={() => setIsExpanded(!isExpanded)}>
                {isExpanded ? 'Hide Submission' : 'View Submission'}
              </Button>
            ) : (
              <UploadDrawer>
                <Button variant={1} className="w-full sm:w-auto justify-center">Upload Files</Button>
              </UploadDrawer>
            )}
          </div>
        </div>
        ...
        {isExpanded && isSubmitted && assignment.submittedFiles && (
          ...
          >
            ...
            <div className="flex flex-col text-left">
              <h3 className="text-h3 font-semibold text-black mb-[20px]">Submitted Files</h3>
              <div className="flex flex-col gap-[16px]">
                {assignment.submittedFiles.map(file => (
                  ...
                  >
                    <span className="text-md font-medium text-black truncate pr-4">{file.name}</span>
                    <div className="flex items-center gap-[10px] shrink-0">
                      <span className="text-md text-medium-gray">{file.size}</span>
