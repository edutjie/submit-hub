  ...
  return (
    <div className="w-full bg-white border-[2px] border-light-gray rounded-[20px] p-[32px] flex flex-col transition-shadow hover:shadow-[0_4px_12px_rgba(0,0,0,0.05)]">
      <div className="flex flex-col lg:flex-row justify-between w-full gap-[32px]">
        <div className="flex flex-col text-left max-w-full lg:max-w-[60%]">
          <h3 className="text-[25px] font-semibold text-black mb-[10px]">{assignment.name}</h3>
          <div className="flex flex-col space-y-[10px] text-[17px] font-semibold text-medium-gray">
            <span>Due: {assignment.dueDate}</span>
            <p className="leading-[145%]">{assignment.description}</p>
          </div>
        </div>

        <div className="flex flex-row items-start gap-[22px] shrink-0">
          <StatusChip status={assignment.status} />
          {isSubmitted ? (
            <Button variant={1} onClick={() => setIsExpanded(!isExpanded)}>
              {isExpanded ? 'Hide Submission' : 'View Submission'}
            </Button>
          ) : (
            <UploadDrawer>
              <Button variant={1}>Upload Files</Button>
            </UploadDrawer>
          )}
        </div>
      </div>
        ...
        {isExpanded && isSubmitted && assignment.submittedFiles && (
          ...
          >
            ...
            <div className="flex flex-col text-left">
              <h3 className="text-[25px] font-semibold text-black mb-[20px]">Submitted Files</h3>
              <div className="flex flex-col gap-[16px]">
                {assignment.submittedFiles.map(file => (
                  ...
                  >
                    <span className="text-[17px] font-medium text-black truncate pr-4">{file.name}</span>
                    <div className="flex items-center gap-[10px] shrink-0">
                      <span className="text-[17px] text-medium-gray">{file.size}</span>
